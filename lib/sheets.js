...
// --- Logs ---
// Sheet columns: A=Name, B=RollNo, C=Event/Company, D=HoursLogged, E=Upvotes, F=Downvotes, G=Status, H=Verification, I=Timestamp
...
export async function addLog(name, roll, company, hours) {
    const sheets = getSheets();
    ...
    await sheets.spreadsheets.values.append({
        ...
        requestBody: {
            values: [[name, roll, company, hours, 0, 0, 'pending', '', timestamp]],
        },
    });
    invalidateAllCaches();
}
...
export async function updateLogStatus(rowIndex, status, approver) {
    const sheets = getSheets();
    // Update Status (G) and Verification (H) columns
    await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `Logs!G${rowIndex}:H${rowIndex}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: {
...
export async function updateLogVotes(rowIndex, upvotes, downvotes) {
    ...
    await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `Logs!E${rowIndex}:F${rowIndex}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: {
