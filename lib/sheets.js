...
// Wrap any promise with a timeout to prevent indefinite hangs (504 errors)
const SHEETS_TIMEOUT_MS = 10000; // 10 seconds
function withTimeout(promise, ms = SHEETS_TIMEOUT_MS) {
    let timeoutId;
    const timeout = new Promise((_, reject) => {
        timeoutId = setTimeout(() => reject(new Error(`Google Sheets API timed out after ${ms}ms`)), ms);
    });
...
/** Normalize PEM from .env (quotes, Windows CRLF, literal \n sequences). */
